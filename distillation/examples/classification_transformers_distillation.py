...
def main(args):
    ...
    loaders = {
        ...
        "valid": DataLoader(datasets["test"], batch_size=args.val_batch_size),
    }

    teacher_model = AutoModelForSequenceClassification.from_pretrained(
        args.teacher_model_name, num_labels=args.num_labels
    ).to(device)
    teacher_model.load_state_dict(torch.load("teacher.pth"))

    student_model = AutoModelForSequenceClassification.from_pretrained(
        args.student_model_name, num_labels=4
    ).to(device)
...
if __name__ == '__main__':
    args = {
        "num_epochs": 5,
        "teacher_model_name": "google/bert_uncased_L-4_H-512_A-8",
        "student_model_name": "google/bert_uncased_L-2_H-128_A-2",
        "tokenizer_name": "google/bert_uncased_L-4_H-512_A-8",
        "train_batch_size": 32,
        "val_batch_size": 32,
