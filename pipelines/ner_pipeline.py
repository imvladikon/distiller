...
class TokenClassificationPipeline(Pipeline):
    ...
    def __init__(
            ...
            return_only_tokens_logits: bool = False,
        ...
        self.return_only_tokens_logits = return_only_tokens_logits
    ...
    def __call__(self, inputs: Union[str, List[str]], **kwargs):
        ...
        for i, sentence in enumerate(_inputs):
            ...
            with self.device_placement():
                ...
                if self.framework == "tf":
                    ...
                    with torch.no_grad():
                        ...
                        outputs = self.model(**tokens, output_hidden_states=self.output_hidden_states)
                        entities = outputs["logits"][0].cpu().numpy()
            ...
            for idx, label_idx in filtered_labels_idx:
                ...
                if not self.return_only_tokens_logits:
                    entity = {
                        "word": word,
            ...
            if self.grouped_entities:
                if not self.output_hidden_states:
                    ...
                    answers += [(self.group_entities(entities), hidden_states)]
            # Append ungrouped entities
            else:
                if not self.output_hidden_states:
                    answers += [entities]
                else:
                    answers += [(entities, hidden_states)]
...
if __name__ == '__main__':
    ...
    model.config.id2label = {idx: l for idx, l in enumerate(label_list)}
    model.config.label2id = {l: idx for idx, l in model.config.id2label.items()}

    ner = NerPipeline(model=model,
                      tokenizer=AutoTokenizer.from_pretrained("bert-base-uncased"),
                      ignore_labels=[],
                      return_only_tokens_logits=True,
                      output_hidden_states=False,
                      return_logits=True,
                      apply_softmax=True)
    out = ner(["EU rejects German call to boycott British lamb", "It was great idea, John"])
    print(out)
    ...
    out = ner(["EU rejects German call to boycott British lamb".split(), "ALEXANDRIA — Lawrence J. “Lorry” Massa, who was chief executive of Rice Memorial Hospital for 14 years, died early Friday morning at his lake home near Alexandria, surrounded by his family, according to a post from his family on his Caring Bridge page.".split()])
    print(out)
